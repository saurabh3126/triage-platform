...
function analyzeFlags(text) {
  ...
  // Bonus: too many exclamation marks
  if (exclamationCount >= 3) score += 10;

  // Bonus: very short text
  if (text.trim().length < 50) score += 5;
  ...
  return { sensational, shouting, unsourced, isHighRisk, riskScore: score, riskLevel };
}
...
module.exports = { analyzeFlags, getSimilarity };